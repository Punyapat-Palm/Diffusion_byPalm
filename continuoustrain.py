...
train_loader = DataLoader(
    train_dataset, batch_size=config['batch_size'], num_workers=4, shuffle=True, persistent_workers=True, pin_memory=True
)
val_loader = DataLoader(
    val_dataset, batch_size=config['batch_size'], num_workers=4, shuffle=True, persistent_workers=True, pin_memory=True
)
...
def train_model(model, train_loader, val_loader, optimizer, epochs, opdir, device):
    ...
    optimizer.load_state_dict(model_path['optimizer_state_dict'])
    best_model_path = model_checkpoint
    best_val_loss = model_path['best_val_loss']
    last_epoch = model_path['epoch']

    print('The model loaded from {}'.format(model_checkpoint))
    logging.info('The model loaded from {}'.format(model_checkpoint))
    print('Continue epoch at {}'.format(last_epoch))
    logging.info('Continue epoch at {}'.format(last_epoch))
    ...
    for epoch in range(last_epoch, epochs):
        ...
        if threshold and epoch - last_epoch > threshold:
            model_path = torch.load(best_model_path, map_location=device, weights_only=True)
            model.load_state_dict(model_path['model_state_dict'])
            last_epoch = epoch + 1
...
if __name__ == "__main__":
    ...
    output_dir = 'result/Stage{}/version_{}'.format(config['diagnosis'], config['version'])
    ...
    logging.basicConfig(filename=os.path.join(output_dir, "outputCT.log"), level=logging.INFO, format='%(asctime)s - %(message)s')
    ...
    print('Duration for continue train: {}'.format(end_time - start_time))
    logging.info('Duration for continue train: {}'.format(end_time - start_time))
